    ...
    return () => {
      return (
        <div class="s-tabs">
              ...
              tabsData.value.map(tab => (
                <li class={tab.id === activeTab.value ? 'active' : ''} onClick={() => changeTab(tab.id)}>
                  ...
                  {/*关闭页签按钮*/}
                  {
                    ...
                    <CloseIcon size="12" color="grey" onClick={(e) => closeTab(tab.id, e)} style="margin-left: 8px;"/>
                  }
                </li>
              ))
            }
          ...
          {/*新增tab内容显示区*/}
          {
            tabsData.value.filter(tab => tab.type === "random").map(tab => (
              <Tab type="random" id={tab.id} title={tab.title}>{tab.content}</Tab>
            ))
          }
        </div>
      )
